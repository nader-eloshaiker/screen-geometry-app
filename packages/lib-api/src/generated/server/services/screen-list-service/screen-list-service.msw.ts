...
export const getUpdateScreenListResponseMock = (): ScreenListResponse => ({
  list: [
    {
      id: 'pVesw1Iu',
      specs: {
...
export const getUpdateScreenListMockHandler = (
  overrideResponse?:
    | ScreenListResponse
    | ((info: Parameters<Parameters<typeof http.put>[1]>[0]) => Promise<ScreenListResponse> | ScreenListResponse)
) => {
  return http.put('*/v1/screens', async (info) => {
    await delay(10)
      ...
      JSON.stringify(
        overrideResponse !== undefined
          ? typeof overrideResponse === 'function'
            ? await overrideResponse(info)
            : overrideResponse
          : getUpdateScreenListResponseMock()
      ),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    )
  })
...
export const getScreenListServiceMock = () => [
  getGetScreenListMockHandler(),
  getCreateScreenListMockHandler(),
  getUpdateScreenListMockHandler(),
]
