...
export const ScreenFormSchema = z.object({
  diagonalSize: z.coerce.number().gt(0, { message: 'Must be greater than 0' }),
  aspectRatio: z.string().regex(/^\d+:\d+$/, { message: 'Must be look like 16:9' }),
  hRes: z.coerce.number().gt(0, { message: 'Must be greater than 0' }),
  vRes: z.coerce.number().gt(0, { message: 'Must be greater than 0' }),
